...
- **Sprint worktree** (`.astra-worktrees/sprint-<N>-<name>/`): created by `/sprint-init`, holding the `feat/sprint-<N>-<name>` branch. *All* feature code and tests for that sprint are written, committed, and pushed inside this worktree. **v5.9+ two-phase merge**: `/pr-merge` invoked from inside the sprint worktree runs Sprint Phase only (commit → push → PR → code review → fix loop) and then stops, instructing the user to `cd` to the main worktree to finalize. The actual `gh pr merge` and worktree removal happen from the main worktree in a second `/pr-merge` invocation (auto-detects the pending sprint PR via `gh pr list --base dev --state open` filtered on `feat/sprint-*` heads). With `--auto` (used by `/autorun` and `/sprint-init --auto`), `/pr-merge` performs the cross-worktree handoff itself so unattended pipelines still complete in one invocation.
...
- **Recommended workflow (v5.8+)**:
  1. `/blueprint <feature-slug>` — invoked from the main worktree (`dev`). Authors the blueprint, runs blueprint-reviewer, commits to dev, then **automatically creates the sprint worktree via Step 6.5** (delegating to `/sprint-init`). **The skill does not cd** — Step 7 output shows the user a concrete `cd .astra-worktrees/sprint-<N>-<slug>/` command to run next.
  ...
  5. `/pr-merge` (Sprint Phase) — invoked inside the sprint worktree → commit · push · PR · code review · fix loop. **Stops after the review loop converges.** Prints a concrete `cd <main-worktree-path>` command for the user to run next.
  6. `cd` to the main worktree (path is printed by step 5).
  7. `/pr-merge` (Main Phase) — auto-detects the pending sprint PR (base=dev, head=feat/sprint-*) → final merge confirmation · `gh pr merge` · auto-remove the sprint worktree → cwd ends in the main worktree (dev). Skip steps 5–7 entirely by using `/pr-merge --auto`, which chains Sprint Phase, the `cd` handoff, and Main Phase in a single invocation (this is what `/autorun` and `/sprint-init --auto` do internally).

  Two skip cases for Step 1's auto-worktree:
  - **Secondary blueprint in the same sprint** (cwd is already inside a sprint worktree) → the blueprint commit lands on the existing sprint branch; no new worktree.
  - **Non-dev/main/master branch** → the skill warns and skips; the user runs `git checkout dev && /sprint-init <slug>` manually.

  **Legacy workflow (pre-v5.8)** still works: explicitly invoke `/sprint-init <slug>` from `dev`, then `cd` into the worktree before authoring the blueprint inside it. The prompt-map Step 1.1 `/blueprint` flow in `/sprint-init` remains intact for this case.
...
| `/blueprint` | Dedicated blueprint (design document) authoring. 10 standard sections (data model · API contract · sequences · pseudocode logic · **HITL Triggers**) — no implementation code. Planner deliverables auto-loaded. Only 1–3 core decisions surface as HITL. Section 10 serves as the HITL guard for `/feature-dev` during implementation. **v5.8+: Step 6.5 auto-creates the sprint worktree via `/sprint-init` delegation at the end of authoring** — the skill does NOT cd; Step 7 output shows the user the `cd <worktree-path>` command to run next. Skipped when already in a sprint worktree or on a non-standard branch. |
...
| `/autorun` | Unattended full pipeline: `/service-planner` → planner-reviewer → design-token-validator → blueprint (Step 6.5 auto-creates the sprint worktree in v5.8+) → blueprint-reviewer → `/sprint-init` (idempotent re-entry — usually skipped) → `/test-scenario` (TDD) → implementation → `/test-run` (5-pass auto-debug) → `/pr-merge --auto` → worktree auto-removal. HITL fires only on real blockers (gh auth, merge conflicts, Critical review issues). `/sprint-init --auto` can enter the same downstream pipeline. |
