...
### Sprint Isolation — Adaptive (v5.16+), Worktree-based (v5.0+)

**v5.16+ adaptive isolation**: a sprint runs **in-place** by default — its `feat/sprint-<N>-<name>` branch is checked out directly in the main worktree (`ISOLATION_MODE=inplace`; no worktree, no `cd`, single-phase `/pr-merge`). It escalates to a sprint-level isolated worktree (`ISOLATION_MODE=worktree`, the v5.0–v5.15 mechanism described below) only when isolation is actually needed: the main worktree is already occupied by another in-place sprint (**the main worktree's current branch is the occupancy signal** — no marker file, crash-safe), the tree is dirty, or `--isolated` is passed. Both modes converge on `SPRINT_BRANCH` + `WT_PATH` (in-place: `WT_PATH` = main root), so the pipeline below is mode-agnostic.

**v5.16+ one-flow**: `/blueprint` and `/sprint-init` no longer stop after their authoring/scaffolding stage — they continue **in the same session** through `/test-scenario` → implementation → `/test-run` → an adversarial verification gate (`loop-verifier` with the fixed SPRINT PRESET rubric; exit = tests PASS ∧ score ≥ 90 ∧ P0 == 0, hard cap 5 iterations) → `/pr-merge`. The user is never instructed to `cd` or start a new session (KV-cache locality: one warm prefix from design through merge). HITL fires only at real judgment points: blueprint design decisions, remaining Critical review issues, final merge confirmation, promotion target. `--scaffold-stop` (sprint-init) / `--design-only` (blueprint) restore the stop-early behavior; the shared pipeline SSoT is `skills/sprint-init/references/auto-pipeline.md`.
...
- **Recommended workflow (v5.16+ — one command, one session)**:
  1. `/blueprint <feature-slug>` — invoked from the main worktree (`dev`). The skill: decides the isolation mode (in-place default / worktree escalation) via `/sprint-init --scaffold-only` → authors the blueprint on the sprint branch → runs blueprint-reviewer → commits → **continues in the same session** (Step 8): `/test-scenario` → implementation → `/test-run` → adversarial verification gate (score ≥ 90 ∧ P0 == 0, max 5 iterations) → `/pr-merge` (merge confirmation + promotion target HITL) → cleanup, session back on `dev`. The blueprint is never committed to `dev` directly; it is the first commit on `feat/sprint-<N>-<slug>` and reaches its integration branch only via `/pr-merge`.
  2. That's it — answer the HITL prompts as they surface (design decisions up front; merge/promotion at the end). `--design-only` stops after the blueprint commit; `/sprint-init --scaffold-stop` scaffolds without running the pipeline; `/sprint-init --resume` recovers a crashed run in a fresh session.

  Skip cases / aborts for Step 1's sprint-context creation:
  - **Secondary blueprint in the same sprint** — cwd inside a sprint worktree (Priority 1) or main worktree already on the in-place sprint branch `feat/sprint-*` (Priority 1.5, v5.16+) → the blueprint commit lands on the existing sprint branch; no new context, and the pipeline is NOT re-launched (the sprint's own flow is already driving).
  - **Non-dev/main/master branch (and not an in-place sprint branch)** → the skill **aborts with error** (per v5.10+ policy). The user must run `git checkout dev` and re-invoke `/blueprint` (Priority 2 guard).
...
> **Version history (v4.x → v5.18.0)**: per-version change rationale lives in [docs/development/changelog-v5.md](docs/development/changelog-v5.md); the sections above authoritatively describe the current (v5.18.0) behavior.
...
| `/blueprint` | Dedicated blueprint (design document) authoring. 10 standard sections (data model · API contract · sequences · pseudocode logic · **HITL Triggers**) — no implementation code. Planner deliverables auto-loaded. Only 1–3 core decisions surface as HITL. Section 10 serves as the HITL guard for `/feature-dev` during implementation. **v5.16+: context-first + one-flow** — the skill first delegates to `/sprint-init --scaffold-only` (adaptive isolation: in-place branch by default, worktree on escalation), authors the blueprint on the sprint branch, then **continues in the same session** through implementation, the adversarial test loop (score ≥ 90 ∧ P0 == 0, max 5), and `/pr-merge`. `--design-only` stops after the commit. Skipped (context reused, no pipeline re-launch) for secondary blueprints. **Aborts with error** when invoked on a non-standard branch (not dev/main/master, not an in-place sprint branch). |
...
| `/autorun` | Mostly-unattended full pipeline: `/service-planner` → planner-reviewer → design-token-validator → blueprint (sprint context created at the start of `/blueprint` via adaptive isolation — in-place branch by default, worktree on escalation; source branch defaults to `dev` under `--auto`) → blueprint-reviewer → `/sprint-init` (idempotent re-entry — usually skipped since `/blueprint` already created the sprint context) → `/test-scenario` (TDD) → implementation → `/test-run` (single-pass; retries are driven by autorun's Stage 7.5 iteration loop up to `--max-iter`) → **Stage 7.6 adversarial verification gate** (loop-verifier SPRINT PRESET, same triple gate as the sprint pipeline: tests PASS ∧ score ≥ 90 ∧ P0 == 0 — FAIL at max iterations skips the merge) → `/pr-merge --auto` (v5.11+: Step 4.5 auto-picks integration branch matching inferred name or creates from dev; **Step 8.4.5 promotion target is HITL even under `--auto` (v5.11.2+) — user picks dev / staging / skip**) → cleanup (worktree removal, or sprint-branch + env-file cleanup under in-place). Routine HITL: max-iteration at start + promotion target at Stage 8.2. Other halts only on real blockers (gh auth, merge conflicts, Critical review issues). `/sprint-init --auto` can enter the same downstream pipeline. |
...
| `/sprint-init` | Sprint scaffolding + adaptive isolation (in-place branch default / worktree escalation) + the shared execution pipeline (see Sprint Isolation section above). `--scaffold-stop` / `--scaffold-only` / `--auto` / `--resume`. |
