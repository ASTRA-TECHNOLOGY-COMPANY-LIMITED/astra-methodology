...
| `/autorun` | Unattended full pipeline: `/service-planner` → planner-reviewer → design-token-validator → blueprint (worktree-first in v5.10+ — sprint worktree created at the start of `/blueprint`) → blueprint-reviewer → `/sprint-init` (idempotent re-entry — usually skipped since `/blueprint` already created the worktree) → `/test-scenario` (TDD) → implementation → `/test-run` (5-pass auto-debug) → `/pr-merge --auto` → worktree auto-removal. HITL fires only on real blockers (gh auth, merge conflicts, Critical review issues). `/sprint-init --auto` can enter the same downstream pipeline. |
