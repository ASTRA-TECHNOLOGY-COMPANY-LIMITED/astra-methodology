...
- **`vibe-coding-animation-guide.md`**: CSS native (View Transitions API, Scroll-Driven Animations, `@starting-style`, `linear()` springs), Framer Motion/GSAP/Lottie/Rive, micro-interactions, performance, 3-tier motion accessibility, Disney 12 principles, AI-generated video backgrounds & seamless loops (§18 — Veo 3.1 bookend/crossfade loop techniques, dual-video infinite-loop player)

These guides are automatically loaded by `/service-planner` (Step 6 HTML mockup generation) and `/landing-page` (Step 0.C), and should be referenced by any skill or workflow that involves UI design, design system work, or animation implementation.
...
- **dev-sync skill guards** (main-worktree-only, 5 skills): `service-planner`, `handoff-publish`, `manual-generator`, `catalog-generator`, `landing-page` — all planning-phase deliverables, so they run in the main worktree (`dev`) before sprint start. `sprint-init`, which was guarded in v4.x, is responsible for worktree creation in v5.0+ and therefore remains main-worktree-only. `test-scenario` and `test-run` had their guards removed in v5.0+ so they can run inside sprint worktrees.
...
6. **check-korean-style.sh** (v5.23.0+) — runs the Korean style gate (`scripts/check-style.py`; rule SoT `docs/development/korean-style.md`) on Korean-bearing files written in target projects: `*.md` and `*.html`/`*.htm` via the `doc` surface (HTML is the primary deliverable format for `/service-planner` mockups, `/landing-page` and `/manual-generator`), source-code comment lines via the `comment` surface (line-marked comments only — block docstrings are out of reach of a line filter). Skips plugin-internal files, files without Hangul, and generated/data artifacts; selftest-guarded (a broken checker reports "unverified" instead of a verdict). Advisory only.
...
| `/manual-generator` | Service URL + project docs → self-contained HTML manual. Real-browser screenshots (ego → Chrome MCP) + annotations. |
| `/catalog-generator` | Product data → self-contained HTML catalog. AI imagery (fect-image) + real-browser screenshots (ego → Chrome MCP) + sales-strategy auto-applied. |
| `/landing-page` (v5.22.0+) | Reference-driven premium landing page as a self-contained HTML package (`landing/{slug}/`). Pipeline: Pinterest animation/layout reference capture (ego → Chrome MCP; Dribbble/Behance/Awwwards → WebSearch fallbacks) → vision-analyzed reference board → reference-anchored design direction (≥ 2 anti-AI distinctive elements; max 2 HITL, `--auto` for zero) → seamless-loop hero video via **Veo 3.1** (bookend first-frame == last-frame technique, ffmpeg self-crossfade fallback, CSS animated hero when Veo absent or `--no-video`) → responsive mobile-first implementation with the dual-video infinite-loop player, poster-first LCP, `prefers-reduced-motion` + WCAG 2.2.2 pause control → `screen-quality-loop` convergence. Default generation budget: 1 still + 1 × 8 s veo-lite clip. |
...
| `/user-test` | Interactive AI-assisted UAT: runs `docs/tests/uat-cases/*.md` case-by-case in a browser (ego → Chrome MCP; backend recorded in `session.json`), records evidence, and emits an HTML report + issues list. Sequential sibling of `/uat-parallel`. |
...
| `/test-run` | Single-pass integration test execution with port isolation and the `ASTRA_TEST_RESULT` machine gate line (retries are driven by the calling pipeline, not internally). Browser backends in detection order: `ego` (default, macOS-only) → `chrome-mcp` (fallback) → `cmux` (legacy). |
...
- **Browser backend policy (v5.21.0+)**: every browser-driving skill resolves its backend in one plugin-wide order — **`ego` (default) → `chrome-mcp` (fallback) → `cmux` (legacy, `/test-run` only)**. The SSoT is [docs/development/browser-backend-policy.md](docs/development/browser-backend-policy.md): detection snippet, capability matrix, per-action mapping across the three backends, ego operating rules, the two escalation cases (performance trace, console messages), the shared deliverable-screenshot recipe, and Task Space naming. Consumers: `/test-run`, `/user-test`, `/manual-generator`, `/catalog-generator`, `/landing-page`. `/uat-parallel` is **not** a consumer — it drives Playwright as a subprocess with its own per-worker `BrowserContext`. Skills reference that document rather than restating the order; change it there once.
- **ego (lite) browser (default backend, optional install)**: a third-party Chromium browser ([citrolabs/ego-lite](https://github.com/citrolabs/ego-lite), macOS-only) driven through Bash heredocs (`ego-browser nodejs`) instead of MCP tool calls. Chosen as the default for **multi-session safety** — an isolated Task Space per agent means no profile `SingletonLock` contention when several Claude Code sessions run concurrently — plus inherited login state for SSO-gated internal services. It is never bundled; when `ego-browser` is absent the skills fall back to Chrome MCP automatically. Its three sharp edges are load-bearing: **login-state inheritance** (an auth scenario can start already signed in, and writes land in the real account), **blank screenshots when `scrollY != 0`**, and **no performance-trace helper** (Step 8 escalates to Chrome MCP). `/test-run`-specific procedures live in `skills/test-run/references/browser-ego.md`, UAT-specific ones in `skills/user-test/references/browser-ego-uat.md`.
- **MCP servers (v5.18+)**: the plugin bundles only `fect-image` / `fect-gpt-image` in `plugin.json`. The `chrome-devtools` MCP server is **not bundled** — it is the fallback browser backend, and the user registers `chrome-devtools-mcp` themselves (in multi-session setups attach one shared instance via `--browser-url` instead of launching per session, to avoid profile `SingletonLock` conflicts). The former `fect-slack` integration (`/slack-import`, `/extract-backlog`, Slack List status sync) was removed in v5.18. The user-level `fect-mcp` server (`veo_*` video generation, `vision_*` analysis) is likewise **not bundled** — `/landing-page` uses its Veo 3.1 tools opportunistically and falls back to a CSS animated hero when they are absent.
...
- **korean-style skill (v5.24.0+)**: `skills/korean-style/` packages the same writing bar for in-session use — compact rules injected at SessionStart (`scripts/sessionstart-notice.sh`) so HITL questions and answers follow them from the first turn, and a self-contained checker to run on Korean deliverables (including Bash-generated files the PostToolUse hook never sees). Its `references/check-korean-style.py` and `references/korean-style.md` are byte-identical with the `proposal-specialist` plugin — fix both together and match the md5. The hook gate above stays the enforcement SoT; the skill does not replace it.
