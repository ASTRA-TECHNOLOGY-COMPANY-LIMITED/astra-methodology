...
# --- i18n strings: single source of truth --------------------------------
# The full vi/en/ko string table lives in the JSON below (SSoT). This script
# holds NO embedded copy — it loads the table at runtime with jq and resolves
# the language via the UAT_LANG env contract. If the file (or jq) is missing,
# or a per-key translation is absent for the chosen language, it falls back to
# English and the report still renders — it never crashes.
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
I18N_JSON="${UAT_I18N_JSON:-$SCRIPT_DIR/../../user-test/references/i18n-strings.json}"

# Resolve LANG_CODE from the UAT_LANG contract (mirrors the doc resolution).
UAT_LANG_RAW=$(printf '%s' "${UAT_LANG:-vi}" | tr '[:upper:]' '[:lower:]')
case "$UAT_LANG_RAW" in
  vi|vie|vietnamese) LANG_CODE=vi ;;
  en|eng|english)    LANG_CODE=en ;;
  ko|kor|korean)     LANG_CODE=ko ;;
  *)                 LANG_CODE=vi ;;
esac

# Flatten the chosen language into { key: value } with per-key English fallback.
# On any failure (jq absent, file missing, malformed JSON) emit ONE warning and
# hand Node an empty map — Node then falls back to the key name per string.
UAT_I18N_STRINGS='{}'
if ! command -v jq >/dev/null 2>&1; then
  echo "[uat-parallel] jq not found — i18n strings unavailable, falling back to key names." >&2
elif [ ! -f "$I18N_JSON" ]; then
  echo "[uat-parallel] i18n strings file not found at $I18N_JSON — falling back to English/key names." >&2
elif ! UAT_I18N_STRINGS=$(jq -c --arg lang "$LANG_CODE" 'map_values(.[$lang] // .en // "")' "$I18N_JSON" 2>/dev/null); then
  echo "[uat-parallel] failed to parse $I18N_JSON — falling back to English/key names." >&2
  UAT_I18N_STRINGS='{}'
fi
export LANG_CODE UAT_I18N_STRINGS
...
// i18n strings — resolved by the bash preamble from the single source of truth
// skills/user-test/references/i18n-strings.json (loaded via jq for LANG_CODE,
// with per-key English fallback). This script holds NO embedded copy.
...
// so the report still renders — it never crashes.
const LANG = String(process.env.LANG_CODE || 'vi');
let RAW_STRINGS = {};
try {
  RAW_STRINGS = JSON.parse(process.env.UAT_I18N_STRINGS || '{}');
} catch {
  RAW_STRINGS = {};
}
const T = new Proxy(RAW_STRINGS, {
  get: (o, k) => (typeof k === 'string' && k in o ? o[k] : k),
});
