...
Write scenario files to `docs/tests/test-cases/sprint-{SPRINT_N}/`, one per feature, named `{feature-name}-e2e-scenarios.md`. Each file has an Overview block (feature / modules / endpoints / tables / blueprint link), then `## Scenario Group N` sections holding `### E2E-{NNN}` entries (Type / Priority / Preconditions / User Journey / Expected Results / Verification Method / Test Data), then a Summary count table.

The full file template with every field: see [references/scenario-file-format.md](references/scenario-file-format.md). Read it before writing the first file. Create the `docs/tests/test-cases/sprint-{SPRINT_N}/` directory if it does not exist.
