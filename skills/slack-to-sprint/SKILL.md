...
1. Step 4.A의 알려진 상태 컬럼 매핑을 참조하여 상태 컬럼 ID와 "시작되지 않음" 옵션 ID를 확인한다
...
1. **알려진 상태 컬럼 매핑 확인**: 아래 테이블에 해당하는 List이면 매핑을 바로 사용한다.
...
선택된 각 Item에 대해 `mcp__fect-slack__slack_list_items_update`를 호출하여 상태를 **"진행중"**으로 변경한다:
