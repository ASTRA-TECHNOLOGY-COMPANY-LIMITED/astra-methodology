...
| 라이브러리 | 버전 | 용도 |
|-----------|------|------|
| {라이브러리명} | {버전} | {애니메이션/제스처/모션} |

### AI 이미지 에셋 (fect-image 생성)
| 이미지 | 용도 | 경로 |
|--------|------|------|
| {이미지명} | {히어로/빈 상태/아이콘/아바타/배경} | `assets/images/{파일명}` |

### 디자인 품질
- Vibe Coding 디자인 가이드 준수 (Anti-AI 미학, 레퍼런스 앵커링, 디자인 토큰)
- Vibe Coding 애니메이션 가이드 준수 (스프링 물리학, GPU 가속, Disney 12원칙)
- UX 인터랙션 패턴 가이드 적용
- fect-image AI 이미지 에셋 적용 (플레이스홀더 없음)
- 프로덕션급 컴포넌트 (/frontend-design)
- 반응형: 모바일/태블릿/데스크톱
- 다크 모드 지원 (이미지 다크모드 대응 포함)
- 접근성: WCAG AA (포커스, 모션 감소, 색상 대비, 터치 타겟)

### 적용된 애니메이션
| 유형 | 구현 방식 | 적용 화면 |
|------|---------|---------|
| 진입 애니메이션 (fadeInUp + stagger) | {CSS/Framer Motion/등} | 전체 |
| 스프링 물리학 이징 | {CSS linear()/spring()} | 전체 |
| 버튼 피드백 (scale + ripple) | CSS | 전체 |
| 스켈레톤 시머 로딩 | CSS keyframe | {목록/대시보드} |
| 카운터 rollup | {JS/라이브러리} | {대시보드/통계} |
| 패럴랙스 | {CSS/라이브러리} | {랜딩/히어로} |
| 이미지 reveal (clip-path/mask) | CSS | {랜딩/상세} |
| 페이지 전환 | {View Transitions/라이브러리} | 전체 |
| 3-tier 모션 접근성 | prefers-reduced-motion | 전체 |
