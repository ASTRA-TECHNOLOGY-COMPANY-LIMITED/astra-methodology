...
### 1. Think Before Coding — Make assumptions explicit
- When uncertain, do not guess — **ask**.
- When multiple interpretations exist, do not silently pick one — **present all of them**.
- When a simpler approach is visible, **push back**.
- Name what is confusing, then stop and ask. *"If something is unclear, stop. Name what's confusing. Ask."*

### 2. Simplicity First — No speculative code
- Do not introduce features, abstractions, "flexibility", or "configurability" that were not requested.
- Do not write error handling for scenarios that cannot occur.
- **Self-check**: *"If a senior engineer saw this, would they call it over-engineered?"* → If so, simplify.
- If you wrote 200 lines and it could have been 50, rewrite it.

### 3. Surgical Changes
- Do not "improve" adjacent code, comments, or formatting.
- Do not refactor what is not broken.
- **Follow the existing style** even if it does not match your preference.
- For unrelated dead code, **mention it but do not delete it**.
- Only remove unused imports/variables introduced by your own change (pre-existing dead code must not be removed without explicit request).
- **Test**: *Every changed line must trace directly to a user request.*

### 4. Goal-Driven Execution — Verifiable success criteria
- Translate imperative tasks into verifiable goals:
  ...
  - "Refactor" → "Ensure tests pass both before and after the refactor"
- For multi-step tasks, state a brief plan with validation checkpoints.

> **Source**: [forrestchang/andrej-karpathy-skills](https://github.com/forrestchang/andrej-karpathy-skills) (MIT) — [Karpathy's observations on LLM coding pitfalls](https://x.com/karpathy/status/2015883857489522876)
>
> **ASTRA auto-builder exception**: *Broad deliverable-generating skills* such as `/service-planner`, `/manual-generator`, `/catalog-generator`, `/handoff-publish`, `/project-init`, `/sprint-init`, `/autorun` produce full-stack deliverables explicitly requested by the user, so they are not bound by the scope limit of "Simplicity First". However, the *individual code* written inside them still follows the four principles.
