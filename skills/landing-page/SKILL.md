---
name: landing-page
description: "Builds a premium, reference-driven marketing landing page as a self-contained HTML package — real-browser Pinterest reference capture, a reference-anchored design direction, a seamless-loop Veo 3.1 hero video (CSS animated fallback), and a responsive mobile-first implementation with a screen-quality convergence pass. Use when the user asks to 'build a landing page', 'make a marketing or promotional page', or wants an animated or video hero background."
...
3. ego mode: one heredoc = one round, and **one capture takes three or more rounds** (navigate → readiness probe → sweep+capture; never compressed into one, or the capture comes out blank — see the reference file); Task Space `astra landing {slug}`; `captureScreenshot` with **absolute** paths; close with `completeTaskSpace('astra landing {slug}', { keep: false })` when done.
...
| Responsive | mobile-first media queries at 768/1120; no horizontal overflow at 390 px (the reference mobile width used throughout Step 5) |
