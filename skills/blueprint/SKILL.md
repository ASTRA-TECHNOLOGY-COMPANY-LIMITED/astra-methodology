---
name: blueprint
description: "Authors a Blueprint (design document) with 10 standard sections — data flow, schema, API contract, sequences, pseudocode logic, HITL Triggers — implementation code excluded, then continues in the same session through the sprint-worktree pipeline: implementation, the adversarial test loop, and /pr-merge. The blueprint is authored and pushed on dev in the main worktree first; --design-only stops after the dev push. Use when designing a feature before implementation or updating an existing blueprint."
