...
8. [블루프린트 기반 스프린트 작성](#8-블루프린트-기반-스프린트-작성)
9. [구현](#9-구현)
10. [테스트 시나리오 작성](#10-테스트-시나리오-작성)
11. [테스트 실행](#11-테스트-실행)
12. [PR / 리뷰](#12-pr--리뷰)
13. [스테이징 브랜치 머지](#13-스테이징-브랜치-머지)
14. [사용자 테스트](#14-사용자-테스트)
15. [메인 브랜치 머지](#15-메인-브랜치-머지)
- [부록](#부록)
...
ASTRA 방법론을 사용하려면 먼저 **astra-methodology 플러그인**을 설치하고, 글로벌 개발환경을 구성해야 합니다. 이 과정은 개발자 머신당 **1회만** 수행하면 됩니다.

> **전제 조건**: [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code)가 설치되어 있어야 합니다.

### 4.1 astra-methodology 플러그인 설치
...
플러그인 설치 후, Claude Code를 실행하여 `/astra-setup` 명령으로 나머지 환경을 자동 구성합니다.

```bash
# Claude Code 실행
claude

# Claude Code 내에서 글로벌 개발환경 셋업 실행
/astra-setup
```
...
1. **글로벌 설정** (`~/.claude/settings.json`)
   ...
   - Permission 모드 설정 (bypassPermissions)
   - Always Thinking 활성화

2. **MCP 서버 등록** (`~/.claude/.mcp.json`)
   - `chrome-devtools` — 브라우저 통합 테스트용
   - `postgres` — 데이터베이스 연동용
   - `context7` — 최신 라이브러리 문서 조회용
...
4. **사전 도구 확인** — Node.js, npm/npx, Git, GitHub CLI 설치 여부 검증

> 셋업 완료 후 결과 리포트가 출력됩니다. 모든 항목이 체크되었는지 확인하세요.

---

## 5. 디자인 시스템 작성

Sprint 0에서 DSA가 주관하여 프로젝트의 **디자인 시스템**을 구축합니다. 디자인 시스템은 AI가 일관된 UI를 생성하기 위한 핵심 기반이며, 디자인 토큰(소스 코드)은 `src/styles/`에, 디자인 문서는 `docs/design-system/`에서 관리합니다.
...
```
src/styles/
└── design-tokens.css       # CSS Custom Properties (컬러, 폰트, 스페이싱) — 소스 코드
...
tailwind.config.js          # Tailwind 기반 프로젝트의 경우 (프로젝트 루트)
```

> **왜 `src/styles/`인가?** `design-tokens.css`는 컴포넌트가 `@import`하여 사용하는 소스 코드입니다. shadcn/ui, MUI, Next.js 등 모든 주요 프레임워크가 토큰/테마 파일을 `src/` 하위에 배치합니다. `docs/`는 사람이 읽는 문서 전용입니다.
...
```
# 디자인 토큰 파일 생성
/feature-dev "src/styles/design-tokens.css에 프로젝트 디자인 토큰을 정의해줘.
...
```
# 디자인 시스템 프리뷰 페이지 생성
/frontend-design "src/styles/design-tokens.css의 디자인 토큰, docs/design-system/의 컴포넌트 스타일 가이드,
...
- 반응형 브레이크포인트별 미리보기
- src/styles/design-tokens.css의 토큰만 사용할 것"
```
...
블루프린트가 완료되면, 이를 기반으로 스프린트를 계획합니다. `/sprint-init` 명령으로 스프린트 문서를 초기화하고, 블루프린트의 기능을 스프린트 백로그로 분배합니다.
...
- 최근 거래 목록 (페이지네이션)
- 다크 모드 기본, 미니멀리스트 스타일
- src/styles/design-tokens.css의 토큰 시스템을 반드시 사용"
...
### 12.4 Gate 2.5: DESIGN-TIME (DSA 검수)
...
### 15.1 Gate 3: BRIDGE-TIME (최종 품질 게이트)
...
#### 시니어 관점 리뷰 (세션 내 수행, v5.25.0+)

페르소나 에이전트(`tester-persona`, `designer-persona`, `developer-persona`)는 v5.25.0에서 제거되었습니다. 대신 세션에서 바로 관점을 요청하면 됩니다 — "테스터 관점에서 검토", "디자이너로서 평가". Claude가 그 자리에서 리뷰를 수행합니다: 엣지 케이스 발굴과 Given-When-Then 제안(테스터), 디자인 시스템·WCAG 2.1 AA·미적 비평(디자이너), 아키텍처·OWASP·기술 부채 리뷰(개발자). 세션에서 수행하면 수정 작업 중에도 자동 적용 스킬이 그대로 살아 있습니다.
...
> 상세 내용은 [5. 디자인 시스템 작성](#5-디자인-시스템-작성)을 참고하세요.

디자인 토큰, 컴포넌트 스타일 가이드, 레이아웃 그리드 시스템을 구축합니다.

#### Step 0.3: Architecture & Standards (Day 3-4)

핵심 기능 설계 문서 생성([6. 블루프린트 작성](#6-블루프린트-작성) 참고), 중앙 DB 설계 문서 작성([7. 데이터베이스 설계](#7-데이터베이스-설계) 참고), 테스트 전략 문서(`docs/tests/test-strategy.md`) 작성을 수행합니다.
